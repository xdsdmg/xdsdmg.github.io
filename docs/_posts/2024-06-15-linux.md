...
之前经常听身边的 SRE 同事聊线上故障及处理过程，觉得很有意思，最近离职后有时间，我也想仔细了解学习下 Linux 系统的状态监控及运维方面的内容，就写了这边文章，本文侧重于记录 Linux 中与监控运维相关的命令行工具的使用方法及一些小技巧。
...
之前工作中有遇到过文件描述符泄露的情况，系统中创建的文件描述符已达到上限。
...
``` Bash
# 查看当前 Shell 环境 fd 软限制
ulimit -n

# 修改当前 Shell 环境 fd 软限制
ulimit -n ${value}

# 查看当前 Shell 环境 fd 硬限制
ulimit -Hn

# 修改当前 Shell 环境 fd 硬限制
ulimit -Hn ${value}
