...
Unigram 较复杂一些，WordPiece 与 BPE 的词表构建是由小到大，而 Unigram 是由大到小，Unigram 会先使用某种启发算法，比如 BPE 或 WordPiece 先生成一个很大的词表，然后进行迭代，每一步迭代会从词表中剔除一个对当前词表对于语料的似然影响最小的词语单元，直至词表达到预定大小。
