...
如何增加指定逻辑卷的磁盘空间？在 Linux LVM 技术中扩展指定分区的磁盘容量，需要按照一定的步骤操作。以下是详细的扩展流程：
...
# 创建新分区
fdisk /dev/sda
# 在 fdisk 中：n (新建) → p (主分区) → 选择分区号 → 设置大小
# 然后：t (修改类型) → 8e (Linux LVM)
# 最后：w (保存)
...
LVM 支持在线扩展，大多数情况下不需要卸载文件系统或重启系统。但建议：
...
- 扩展前检查文件系统类型
- 对于根分区，操作要格外小心
- 某些旧版本的文件系统可能需要先卸载才能调整大小

通过以上步骤，你可以安全地扩展 LVM 分区的磁盘容量。